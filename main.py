...
def format_signal(signal):
    ...
    return f"""
{signal['emoji']} <b>BIGSCRUZ CRT SIGNAL</b>

Direction: <b>{signal['direction']}</b>
Entry Price: <b>${signal['entry']:,.2f}</b>

📐 CRT Range:
CRTH: ${signal['crth']:,.2f}
CRTL: ${signal['crtl']:,.2f}

🏦 Malaysian S/R Hit: ${signal['sr_level']:,.2f}
🎯 Sweep: {signal['sweep']}
...
def main():
    ...
    while True:
        try:
            ...
            print(f"  S/R levels found — Support: {len(support_levels)} | Resistance: {len(resistance_levels)}")
            ...
            if signal:
                ...
                print(f"  ⏸ No CRT setup yet. Waiting for sweep...")
