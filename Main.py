...
# MT5 symbol map — Railway/Jarvis uses these names to match XM's symbols
MT5_SYMBOL_MAP = {
    "XAU/USD": "XAUUSD",
    "BTC/USD": "BTCUSD",
    "ETH/USD": "ETHUSD",
...
GROQ_VISION_MODELS = [
    ...
    for m in os.environ.get(
        ...
        # Spotted live in the Groq console. Vision support unconfirmed — if it
        # is text-only it will reject the image and the next candidate runs.
        "qwen/qwen3.6-27b",
    ).split(",")
    if m.strip()
]
...
# NOTE: MT5 itself runs on your Windows laptop via a separate mt5_bridge.py
# Jarvis on Railway sends signals — the bridge on your laptop executes them
# When AUTO_TRADE=true in Railway vars, Jarvis posts signals to the bridge URL
# ─────────────────────────────────────────────────────────────────────────────
...
def execute_trade_via_bridge(signal: TradeSignal):
    ...
    try:
        ...
        log.error(f"execute_trade_via_bridge error: {e}")
...
async def cmd_autotrade(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0].lower() in ("on", "off"):
        AUTO_TRADE = args[0].lower() == "on"
        status = "ENABLED ✅" if AUTO_TRADE else "DISABLED ❌"
        bridge = f"Bridge: {MT5_BRIDGE_URL}" if MT5_BRIDGE_URL else "Bridge: NOT SET — add MT5_BRIDGE_URL to Railway"
        await update.message.reply_text(
            f"🔄 Auto-trading {status}\n"
            f"Risk: {RISK_PERCENT}% | Max lot: {MAX_LOT}\n"
            f"{bridge}"
        )
    else:
        status = "ENABLED ✅" if AUTO_TRADE else "DISABLED ❌"
