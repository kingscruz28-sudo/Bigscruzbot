...
# Assets: Gold, Silver, Oil, ETH, USDJPY, US100, SOL, BTC
...
ASSETS = {
“XAU/USD”: {“name”: “Gold”,      “pip”: 0.10,  “target_pips”: 100, “source”: “twelve”},
“XAG/USD”: {“name”: “Silver”,    “pip”: 0.01,  “target_pips”: 100, “source”: “twelve”},
“WTI/USD”: {“name”: “Oil (WTI)”, “pip”: 0.01,  “target_pips”: 100, “source”: “twelve”},
“ETH/USD”: {“name”: “Ethereum”,  “pip”: 0.10,  “target_pips”: 100, “source”: “twelve”},
“USD/JPY”: {“name”: “USD/JPY”,   “pip”: 0.01,  “target_pips”: 100, “source”: “twelve”},
“NDX”:     {“name”: “US100”,     “pip”: 1.0,   “target_pips”: 100, “source”: “twelve”},
“SOL/USD”: {“name”: “Solana”,    “pip”: 0.10,  “target_pips”: 100, “source”: “twelve”},
“XBTUSD”:  {“name”: “Bitcoin”,   “pip”: 1.0,   “target_pips”: 100, “source”: “kraken”},
}

SWEEP_BUFFER    = 0.0005
SR_LOOKBACK     = 50
CHECK_INTERVAL  = 120   # 2 mins — stays within Twelve Data free tier
SIGNAL_COOLDOWN = 300
API_CALL_DELAY  = 8     # seconds between each asset to avoid rate limits
...
if asset_info[“source”] == “kraken”:
mins = 60 if interval == “1h” else 1
return get_candles_kraken(symbol, mins)
else:
tf = “1h” if interval == “1h” else “1min”
return get_candles_twelve(symbol, tf)
...
if swept_low and closed_back_above and near_support:
    ...
    sl = crtl - (50 * pip)
    rr = round((tp - curr_close) / max(curr_close - sl, 0.0001), 1)
    return {"direction": "LONG", "entry": curr_close,
            "crth": crth, "crtl": crtl, "sr_level": support_hit,
            "sweep": "LOW SWEPT", "tp": tp, "sl": sl, "rr": rr}
...
if swept_high and closed_back_below and near_resistance:
    tp = curr_close - (target_pips * pip)
    sl = crth + (50 * pip)
    rr = round((curr_close - tp) / max(sl - curr_close, 0.0001), 1)
    return {"direction": "SHORT", "entry": curr_close,
            "crth": crth, "crtl": crtl, "sr_level": resistance_hit,
            "sweep": "HIGH SWEPT", "tp": tp, "sl": sl, "rr": rr}
...
def main():
print(“BIGSCRUZ MULTI-ASSET CRT BOT STARTING…”)
